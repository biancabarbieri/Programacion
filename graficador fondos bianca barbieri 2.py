...
def main():
    ...
    imagenTkPortada = ImageTk.PhotoImage(imagenRedimensionada)
    ventana.portada=imagenTkPortada
    etiquetaPortada= tk.Label(ventana, image=imagenTkPortada)
    etiquetaPortada.place(x=0,y=0,relwidth=1, relheight=1)

    fondo= descargarFondo()

    imagenRedimensionada2 = fondo.resize((ancho, alto), Image.Resampling.LANCZOS)
    
    imagenTkFondo= ImageTk.PhotoImage(imagenRedimensionada2)
    ventana.fondo= imagenTkFondo
    etiquetaFondo= tk.Label(ventana, image=imagenTkFondo)
